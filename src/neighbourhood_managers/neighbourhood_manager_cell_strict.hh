...
#ifndef NEIGHBOURHOOD_MANAGER_CELL_STRICT_H
#define NEIGHBOURHOOD_MANAGER_CELL_STRICT_H
...
namespace rascal {
  ...
  class NeighbourhoodManagerCellStrict;

  //! traits specialisation for Lammps manager
  template <>
  struct NeighbourhoodManager_traits<NeighbourhoodManagerCellStrict> {
    constexpr static int Dim{3};
    constexpr static int MaxLevel{2};
  };
  class NeighbourhoodManagerCellStrict: public NeighbourhoodManagerBase<NeighbourhoodManagerCellStrict>
  {
  public:
    using traits = NeighbourhoodManager_traits<NeighbourhoodManagerCellStrict>;
    using Parent = NeighbourhoodManagerBase<NeighbourhoodManagerCellStrict>;
    ...
    //! Default constructor
    NeighbourhoodManagerCellStrict() = default;

    //! Copy constructor
    NeighbourhoodManagerCellStrict(const NeighbourhoodManagerCellStrict &other) = delete;

    //! Move constructor
    NeighbourhoodManagerCellStrict(NeighbourhoodManagerCellStrict &&other) = default;

    //! Destructor
    virtual ~NeighbourhoodManagerCellStrict() = default;

    //! Copy assignment operator
    NeighbourhoodManagerCellStrict& operator=(const NeighbourhoodManagerCellStrict &other) = delete;

    //! Move assignment operator
    NeighbourhoodManagerCellStrict& operator=(NeighbourhoodManagerCellStrict &&other) = default;
  ...
  void NeighbourhoodManagerCellStrict::build(const Eigen::MatrixXd& positions,
                                        ...
                                        const std::array<bool,3>& pbc, const double& cutoff_max)
    {
      ...
      for (int id{0} ; id<Ncenter ; ++id) {
          this->centers.push_back(NeighbourhoodManagerCellStrict::AtomRef_t(this->get_manager(),id));
      ...
      //std::vector<NeighbourhoodManagerCellStrict::vVector3d> offsetlist(Natom);
              ...
              zipfor(jbin_id,jshift eachin bin2neighbin[ii][jj][kk],bin2neighbin_shift[ii][jj][kk]){
                ...
                for (auto jneigh:bin2icenter[jbin_id[0]][jbin_id[1]][jbin_id[2]]){
                  ...
                  if (cutoff_max2 > dist2){
                    this->neighlist[icenter].push_back(NeighbourhoodManagerCellStrict::AtomRef_t(this->get_manager(),count));
      ...
      for (auto center: centers ){
        ...
        NeighbourhoodManagerCellStrict::vVector3d  offsets;
        ...
        for (auto id: neighbours_index){
          neighlist[center.get_index()].push_back(NeighbourhoodManagerCellStrict::AtomRef_t(this->get_manager(),id));
        }
  ...
  void NeighbourhoodManagerCellStrict::update(const Eigen::MatrixXd& positions,
                                        const Eigen::MatrixXd& cell,
                                        const std::array<bool,3>& pbc, const double& cutoff_max)
  {
    bool some_condition{false};
    if (some_condition){
      NeighbourhoodManagerCellStrict::build(positions,cell,pbc,cutoff_max);
    }
  }
  /* ---------------------------------------------------------------------- */

  template<int Level, int MaxLevel>
  inline int NeighbourhoodManagerCellStrict::
  ...
  template <>
  inline int NeighbourhoodManagerCellStrict:: template
...
#endif /* NEIGHBOURHOOD_MANAGER_CELL_STRICT_H */
