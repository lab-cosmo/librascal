...
namespace rascal {
  ...
  class RepresentationManagerSphericalExpansion
   ...
   public:
    ...
    RepresentationManagerSphericalExpansion(Manager_t & sm,
        ...
        : structure_manager{sm}, expansions{sm} {
    ...
    //   return this->expansions[center];
    // }

    // Eigen::MatrixXd get_soap_vector(const size_t & i_center) {
    //   return this->expansions[i_center];
    // }
    ...
    void print_soap_vector(const ClusterRefKey<Order, Layer> & center,
      ...
      auto keys{this->expansions.get_keys(center)};
      for (auto& key :keys) {
        stream << "type = " << key << std::endl;
        for (size_t radial_n{0}; radial_n < this->max_radial; ++radial_n) {
          stream << "n = " << radial_n << std::endl;
          stream << this->expansions(center, key).row(radial_n) << std::endl;
        }
      }
    }
    ...
    data_t& get_representation_sparse_raw_data() {
      return this->expansions.get_raw_data();
    }

    size_t get_feature_size() {
      return this->expansions.get_nb_comp();
    }

    size_t get_center_size() { return this->expansions.get_nb_item(); }
  ...
  RepresentationManagerSphericalExpansion<Mngr>::compute_by_gaussian_sigma() {
    ...
    this->expansions.clear();
    this->expansions.set_shape(n_row, n_col);
    // this->expansions.resize();

    for (auto center : this->structure_manager) {

      // auto& expansion = this->expansions[center];
...
//       // input_data_t expansion{};
//       // key_t center_type{center.get_atom_type()};
//       // expansion.initialize_to_zeros(center, center_type);
//       // expansion.emplace(
//       //     std::make_pair(center_type, dense_t::Zero(n_row, n_col)));
//       // for (auto neigh : center) {
//       //   key_t neigh_type{neigh.get_atom_type()};
