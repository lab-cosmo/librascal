...
namespace rascal {
  ...
  namespace internal {
    enum class SOAPInvariantType { RadialSpectrum, PowerSpectrum, BiSpectrum };
  }  // namespace internal
  ...
  class RepresentationManagerSOAPInvariant : public RepresentationManagerBase {
   public:
    ...
    void set_hyperparameters(const Hypers_t & hypers) {
      ...
      if (this->soap_type_str.compare("PowerSpectrum") == 0) {
        this->soap_type = internal::SOAPInvariantType::PowerSpectrum;
      } else if (this->soap_type_str.compare("RadialSpectrum") == 0) {
        this->soap_type = internal::SOAPInvariantType::RadialSpectrum;
      } else if (this->soap_type_str.compare("BiSpectrum") == 0) {
        this->soap_type = internal::SOAPInvariantType::BiSpectrum;
   ...
   protected:
    ...
    internal::SOAPInvariantType soap_type{};
  ...
  void RepresentationManagerSOAPInvariant<Mngr>::compute() {
    using internal::SOAPInvariantType;
    switch (this->soap_type) {
    case SOAPInvariantType::RadialSpectrum:
      this->compute_radialspectrum();
      break;
    case SOAPInvariantType::PowerSpectrum:
      this->compute_powerspectrum();
      break;
    case SOAPInvariantType::BiSpectrum:
  ...
  template <class Mngr>
  void RepresentationManagerSOAPInvariant<Mngr>::precompute_w3js() {
                    ...
                    for (size_t l1 = 0; l1 < this->max_angular+1; l1++) {
                      for (size_t l2 = 0; l2 < this->max_angular+1; l2++) {
                        for (size_t l3 = 0; l3 < this->max_angular+1; l3++) {
                          ...
                          coef2c /= std::sqrt(2.0);
                          coef3c /= std::sqrt(2.0);
                          //The descriptor components are purely real or imaginary
