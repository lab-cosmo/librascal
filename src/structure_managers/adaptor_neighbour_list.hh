...
public:
  ...
  PeriodicImages(const std::array<int, Dim> &origin,
                 const std::array<int, Dim> &nrepetitions, const size_t &ntot)
  ...
  public:
    ...
    //! constructor
    explicit iterator(const PeriodicImages &periodic_images, bool begin = true)
        : periodic_images{periodic_images}, index{
                                                begin
                                                    ? 0
                                                    : periodic_images.size()} {}
...
public:
  ...
  explicit IndexContainer(const std::array<int, Dim> &nboxes) : nboxes{nboxes} {
  ...
  const std::vector<int> &operator[](const std::array<int, Dim> &ccoord) const {
    auto index = get_index(this->nboxes, ccoord);
...
public:
  ...
  using Parent = StructureManager<AdaptorNeighbourList<ManagerImplementation>>;
  ...
  inline size_t get_nb_clusters(size_t cluster_size) const {
    switch (cluster_size) {
    case 1: {
      return this->n_centers + this->n_ghosts; // TODO(markus) here +n_ghosts?
      break;
    }
    case 2: {
  ...
  inline int get_cluster_neighbour(const ClusterRefKey<Order, Layer> &cluster,
                                   size_t index) const {
...
protected:
  ...
  inline void add_entry_number_of_neighbours() { this->nb_neigh.push_back(0); }
