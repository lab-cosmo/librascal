...
namespace rascal {
  ...
  class TypedProperty : public PropertyBase {
   ...
   public:
    ...
    // TODO(alex) sketch how Order=1 could be accessed with Cluster Order>1
    // template <size_t CallerOrder, size_t CallerLayer>
    // inline std::conditional<std::enable_if<std::is_same<CallerOrder, order_1_t>::is_same, as_type(CallerOrder),,>
    //inline reference operator[](const ClusterRefKey<CallerOrder, CallerLayer> & id) {
    //  static_assert(CallerLayer >= PropertyLayer,
    //                "You are trying to access a property that does not exist at"
    //                "this depth in the adaptor stack.");
    //  return this->operator[](id.get_cluster_index(CallerLayer));
    ...
    //template <size_t CallerOrder>
    //std::conditional<std::is_same<Order1_type, as_type(CallerOrder)>
