...
namespace rascal {
  ...
  decltype(auto) downcast_kernel_impl(
    ...
    return std::static_pointer_cast<internal::KernelImpl<Type>>(
        kernel_impl);
  }
  ...
  class Kernel {
   public:
    ...
    Kernel(const Hypers_t& hypers) {
      ...
      if (kernel_type_str.compare("Cosine") == 0 ) {
        ...
        throw std::logic_error(
              "Requested Kernel \'" + kernel_type_str +
    ...
    math::Matrix_t compute(const Calculator& calculator,
      ...
      switch (this->target_type) {
        case TargetType::Structure: {
          return this->compute_helper<Property_t, TargetType::Structure>(representation_name, managersA, managersB);
          break;
        }
        case TargetType::Atom: {
          return this->compute_helper<Property_t, TargetType::Atom>(representation_name, managersA, managersB);
          break;
        }
        default:
    ...
    template<class Property_t, internal::TargetType Type, class StructureManagers>
    inline math::Matrix_t compute_helper(const std::string& representation_name,
                        const StructureManagers& managersA,
                        const StructureManagers& managersB) {
      using internal::KernelType;
      ...
      switch (this->kernel_type) {
        case KernelType::Cosine: {
          ...
          return kernel->template compute<Property_t, Type>(managersA, managersB, representation_name);
          break;
        }
        default:
          throw std::logic_error("The combination of parameter is not handdled.");
