/**
 * file   test_neighbourhood_strict.cc
 *
...
namespace rascal {
  ...
  BOOST_AUTO_TEST_SUITE(ManagerStrictTests);
  /* ---------------------------------------------------------------------- */
  BOOST_FIXTURE_TEST_CASE(manager_cell_constructor_test, ManagerFixture<NeighbourhoodManagerStrict>){}
  
  /* ---------------------------------------------------------------------- */
  
  BOOST_FIXTURE_TEST_CASE(manager_test, ManagerFixture<NeighbourhoodManagerStrict>) {
    ...
    for (auto center: manager) {
      BOOST_CHECK_EQUAL(atom_counter, center.get_index());
      ++atom_counter;
      for (int ii{3};ii<3;++ii){
        BOOST_CHECK_EQUAL(positions_test(ii,center.get_index()),center.get_position()[ii]);
      }
      //int icenter{center.get_atom_index()};
      for (auto type: center) {
        for (auto neigh : type) {
          auto pair_offset{neigh.get_global_index()};
          /*int ilin{neigh.get_index()};
          int ineigh{neigh.get_atom_index()};
          int itype{neigh.get_atom_type()};
          double d{(neigh.get_position() + cell * neigh.get_atom_shift() - center.get_position()).norm()};*/
          if (verbose) {
            std::cout << "pair (" << center.get_atoms().back().get_index()
                      << ", " << neigh.get_atoms().back().get_index()
