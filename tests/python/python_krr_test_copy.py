...
class TestKRR(unittest.TestCase):
    ...
    def test_local_energies_sum_to_total_energy(self):
        ...
        KJ = self.model.kernel._kernel.compute_local(
                self.model.kernel._representation,
                manager.managers,
                self.model.X_train._sparse_points)
        ...
        self.assertTrue(
                np.allclose(KI,
                np.sum(KJ.reshape(-1,10,30),axis=1)))
        
        self.assertTrue(np.allclose(ei , np.sum(ej.reshape(-1,10),axis=1)))
        
    def compute_numerical_stress(self,frame):
        h_disp = 1e-6
        ...
        numerical_stress = np.zeros((len(frame),9))
        for i in range(9):
            frame_displaced_plus = displace_strain_tensor(
                copy.deepcopy(frame),
                matrix_indices_in_voigt_notation[i][0],
                matrix_indices_in_voigt_notation[i][1],
                h_disp,
            )
            ...
            calculator = self.model.get_representation_calculator()
            manager_plus = calculator.transform(frame_displaced_plus)
            manager_minus = calculator.transform(frame_displaced_minus)
            
            KJ_plus = self.model.kernel._kernel.compute_local(
                    self.model.kernel._representation,
                    manager_plus.managers,
                    self.model.X_train._sparse_points)
            
            KJ_minus = self.model.kernel._kernel.compute_local(
                    self.model.kernel._representation,
                    manager_minus.managers,
                    self.model.X_train._sparse_points)
            
            ej_plus  = np.dot(KJ_plus, self.model.weights)
            ej_minus  = np.dot(KJ_minus, self.model.weights)
            numerical_stress[:,i] = (ej_plus - ej_minus) / (2 * h_disp)
        return numerical_stress / frame.get_volume()
    
    def test_local_stress_with_numerical_stress(self):
        """ Tests if the local stress contribution summed up match the global one"""
        ...
        with open('output.log','w') as logfile:
            ...
            for frame in self.frames[:]:
                ...
                print(f'{local_stress_matrix=}',file=logfile)
                numerical = self.compute_numerical_stress(frame).reshape(-1, 3, 3)
                print(f'{numerical=}',file=logfile)
                print(f'{numerical-local_stress_matrix=}',file=logfile)
                print(f'{1e-08+1e-5*np.abs(numerical)=}',file=logfile)
                #print(i)
                #self.assertTrue(
                # np.allclose(local_stress_matrix,
                # numerical,atol=1e-10,rtol=1e-6))
                i+=1
