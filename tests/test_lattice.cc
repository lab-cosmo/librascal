...
namespace proteus {
  ...
  BOOST_FIXTURE_TEST_CASE(transformation_matrix_test, ManagerFixture_lattice) {
    ...
    cartesian2scaled_true <<  0.161550888529887,0.00              ,0.000            ,
    ...
    scaled2cartesian_true <<  6.190000000000000,0.00             ,0.00             ,
                              2.409999999999999,6.150000000000001,0.00             ,
                              0.210000000000000,1.019999999999999,7.310000000000000; 
    
    for (int jj{0};jj<3;++jj) {
      for (int ii{0};ii<3;++ii) {
        BOOST_CHECK_CLOSE(cartesian2scaled_true(ii,jj), cartesian2scaled(ii,jj),lattice_tol);
        BOOST_CHECK_CLOSE(scaled2cartesian_true(ii,jj), scaled2cartesian(ii,jj),lattice_tol);
      }
    }
    
  }

  BOOST_FIXTURE_TEST_CASE(cartesian2scaled_test, ManagerFixture_lattice) {
    ...
    Eigen::MatrixXd positions_sc;//get_scaled2cartesian
    ...
    for (int jj{0};jj<22;++jj) {
      for (int ii{0};ii<3;++ii) {
        BOOST_CHECK_CLOSE(positions_sc_true(ii,jj), positions_sc(ii,jj),lattice_tol);
      }
    }
