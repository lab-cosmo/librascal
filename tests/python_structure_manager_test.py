...
sys.path.insert(0, '../tests/')  # noqa

from test_utils import load_json_frame, BoxList, Box  # noqa
import rascal.lib._rascal as rc  # noqa
...
class TestNL(unittest.TestCase):
    ...
    def test_update(self):
        manager = rc.Adaptor.NeighbourList_Centers(
            self.manager, self.max_cutoff)
        manager.update()
...
class TestNLStrict(unittest.TestCase):
    def setUp(self):
        ...
        self.managerC.update(np.array(self.positions.T, order='F'),
                             ...
                             np.array(self.cell.T, order='F'),
                             self.pbcs[0].reshape(3, 1))
        self.manager = rc.Adaptor.NeighbourList_Centers(
    ...
    def test_a_update(self):
        manager = rc.Adaptor.Strict_NeighbourList_Centers(
            self.manager, self.max_cutoff)
        manager.update()
    ...
    def test_manager_iteration_2(self):
        ...
        for ii, center in enumerate(manager):
            ...
            self.assertTrue(np.allclose(
                ref_dists[ref_sort_ids], dists[sort_ids]))
            self.assertTrue(np.allclose(
                ref_dirVecs[ref_sort_ids], dirVecs[sort_ids]))
