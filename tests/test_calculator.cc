...
namespace rascal {
  ...
  BOOST_FIXTURE_TEST_CASE_TEMPLATE(spherical_expansion_radial_derivative, Fix,
    ...
    GradientTestFixture test_data{"../reference_data/unknown/radial_derivative_test.json"};
    ...
    for (auto manager : managers) {
      for (auto hyper : hypers) {
        ...
        RepresentationManagerGradientFixture<typename Fix::Representation_t,
                                             typename Fix::Manager_t>
            grad_fix("../reference_data/unknown/spherical_expansion_gradient_test.json",
                     manager, calculator);
