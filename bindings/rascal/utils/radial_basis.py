...
def radial_basis_functions_dvr(radial_grid, max_radial, interaction_cutoff, gaussian_sigma):
    """
...
def get_radial_basis_covariance(spherical_expansion, spherical_expansion_by_species):
    ...
    n_environments = len(spherical_expansion_by_species[list(spherical_expansion_by_species)[0]])
    ...
    for species in spherical_expansion_by_species.keys():        
        ...
        spherical_expansion_coefficients = spherical_expansion_by_species[species].reshape(
            (n_environments, max_radial, (max_angular + 1) ** 2)
        )
        invariant_covariance_matrices[species] = np.zeros((max_angular+1, max_radial, max_radial))
...
def get_radial_basis_pca(covariance_matrices):
    ...
    for species in covariance_matrices.keys():
        ...
        principal_vectors[species] = np.zeros((max_angular_plusone, max_radial, max_radial))
