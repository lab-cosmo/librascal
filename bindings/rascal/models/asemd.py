...
class ASEMLCalculator(Calculator, BaseIO):
    ...
    def __init__(self, model, representation, **kwargs):
        ...
        if len(model) != len(representation):
            raise ValueError(
                "should provide a representation per model. len(model)!=len(representation)"
            )
    ...
    def calculate(
        ...
        for i in range(len(self.manager)):
            ...
            if "forces" in properties:
                if i == 0:
                    self.results["forces"] = self.model[i].predict_forces(
                        self.manager[i]
                    )
                else:
                    self.results["forces"] += self.model[i].predict_forces(
            ...
            if "stress" in properties:
                if i == 0:
                    self.results["stress"] = (
                        self.model[i].predict_stress(self.manager[i]).flatten()
                    )
                else:
                    self.results["stress"] += (
                        self.model[i].predict_stress(self.manager[i]).flatten()
