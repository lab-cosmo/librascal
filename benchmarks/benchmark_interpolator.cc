 ...
 class MyFixture : public ::benchmark::Fixture {
  ...
  BENCHMARK_DEFINE_F(InterpolatorDataFixture, BenchTests2)(benchmark::State &state) {
    ...
    auto intp{Interpolator <
      ...
      SearchMethod<SearchMethod_t::Hunt>
        >()};
    intp.initalize(this->func, this->x1, this->x2, this->mean_error_bound); 
    for (auto _ : state) {
  ...
  //BENCHMARK_REGISTER_F(MyFixture, FooTest2);
    ...
    void BM_Intp(benchmark::State& state, double x1, double x2, int log_mean_error_bound, int nb_points) {
      ...
      state.counters.insert({{"x1",x1},{"x2",x2},{"log(mean_error_bound)",log_mean_error_bound},{"nb_points",nb_points},{"GridSize",intp.grid_rational.grid_size}});
    }

      //std::vector<int> nb_points2 = {1000,2000};
      //for (size_t i{0}; i <nb_points2.size();i++) {
      //}
    BENCHMARK_CAPTURE(BM_Intp, basic_test, 0,5,-3,1000);
    BENCHMARK_CAPTURE(BM_Intp, basic_test, 0,5,-3,1000);
